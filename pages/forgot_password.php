...
$whatsappAdmin = "https://wa.me/2348012345678"; // update if needed
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' || $forceRetry) {
    ...
    if ($check->num_rows == 0) {
        ...
        // 2. Look for an existing non-expired reset
        $stmt = $conn->prepare("SELECT id, token, sent_status, used, expires, created_at, sent_at, used_at
                                FROM password_resets
                                WHERE email = ? ORDER BY created_at DESC LIMIT 1");
        $stmt->bind_param('s', $email);
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($reset_id, $token, $sent_status, $used, $expires, $created_at, $sent_at, $used_at);
        ...
        if ($stmt->num_rows > 0) {
            ...
            // FORCE RETRY: Mark old resets as used/invalid if requesting again
            if ($forceRetry) {
                ...
                $status = "pending";
                $message = "Your new request has been received and is pending.<br>
                           <span class='meta-small'>Manual requests may take a few hours.</span>";
                $showForm = false;
            } else if ($used == 1 || $sent_status === "used" || $sent_status === "invalid") {
                $status = "used";
                $message = "Your previous reset link is no longer valid. <a href='forgot_password.php?retry=1&email=" . urlencode($email) . "'>Request another link</a>.";
                ...
                $message = "Reset link was sent to your email.<br>
                        <a href='forgot_password.php?retry=1&email=" . urlencode($email) . "'>Request another link</a> if you didn't receive it.";
                $showForm = false;
            } else {
                // expired or reusable, allow new link
                $status = "expired";
                $message = "Previous reset link expired or invalid.<br>
                    <a href='forgot_password.php?retry=1&email=" . urlencode($email) . "'>Request a new one</a>.";
            }
        } else {
            // First time for this user or expired
            $token = bin2hex(random_bytes(32));
            $expires = date('Y-m-d H:i:s', strtotime('+24 hours'));
            $created = date('Y-m-d H:i:s');
            $sent_status = 'pending';
...
<head>
    ...
    .a-meta-row { display: flex; justify-content: space-between; gap:9px; margin-bottom:7px;}
    .a-meta-label { color: #858fa4; font-weight: 600; font-size: 0.97em;}
    .a-meta-value { color: #214d35; font-weight: 700; font-size: 0.99em;}
    .a-msg { text-align:center;margin:22px 1px 7px 1px; font-size: 1.08em;font-weight:500;}
    .meta-small { color:#888; font-size:.94em;}
    .a-footer {margin-top:17px;font-size:.98em;text-align:center;color:#175233;}
    .cta-button {background: linear-gradient(90deg,#10b859 70%,#19ffc9 120%);
    ...
    .back-link {display:block;color:#10b859;text-align:center;margin-top:17px;font-weight:600;text-decoration:none;}
    .admin-contact {color:#099d44; font-weight:700; display:inline-block; text-decoration:none; margin-left:6px;}
    .admin-contact:hover { text-decoration:underline; color:#1ea958;}
    .float-contact-btn {
      position: fixed; right: 20px; bottom: 24px; background: #25d366;
      color: #fff; z-index: 2000; padding: 15px 19px 15px 17px;
      border-radius: 50%; font-size: 34px; box-shadow: 0 3px 20px #0d7d383a;
      border: 2.2px solid #fff; display: flex; align-items: center; justify-content: center;
      transition: background .12s, box-shadow .13s, transform .14s; text-decoration: none;
    }
    .float-contact-btn:hover { background: #21b75c; color: #f4fff9; box-shadow: 0 6px 24px #0d7d3840; transform: scale(1.08);}
    @media (max-width: 600px) { .float-contact-btn { font-size: 27px; padding: 13px 15px; } }
    </style>
...
<body>
    ...
    <a class="float-contact-btn" href="<?php echo $whatsappAdmin; ?>" target="_blank" title="Chat on WhatsApp">
